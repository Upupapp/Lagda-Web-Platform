...
## Resolved by BACKEND-41 — final sealing and completion

- **Composition:** merged signed pages first, completion certificate appended
  last. One downloadable file carrying its own completion record.
- **Composition owner:** inside `@lagda/sealing`, via a semantic
  `completionCertificate` byte input on `SealRequest` — never an
  `appendPages()` API.
- **Final artifact kind:** `sealed` (the existing `ARTIFACT_TYPES` member).
- **Seal scheme / version / digest:** `hash-evidence` / `1` / `sha-256`. The
  existing constants; nothing renamed or redesigned.
- **Final hash semantics:** ONE authoritative digest — SHA-256 over the exact
  stored final bytes. The artifact row is the source of truth; the seal record
  references it. No competing digests.
- **`originalDocumentHash`:** the request's FROZEN SOURCE artifact digest —
  **never** the merged candidate. The §0 trap; the seam was renamed so the
  mistake now reads as wrong at the call site.
- **Verification identity:** created in the finalization transaction. One per
  completed request; retry-stability comes from the completion row's uniqueness
  rather than from a reservation mechanism. Removed from `SealRequest`, where
  nothing read it.
- **Visual seal mark / QR:** NOT_IN_PRODUCT. Omitting the QR also removes the
  ordering problem of needing an identity before the bytes are sealed.
- **Completion record:** request-scoped, unique per request, with
  `merged_artifact_id` added so the provenance chain reads from one row.
- **Document ↔ completed artifact:** by relation, request-scoped. No
  `document.final_artifact_id`, because one document may have several requests
  and the second would overwrite the first.
- **Post-completion lockout:** BOTH — deny by state (the control) and revoke
  grants and sessions (defence in depth). Tested independently.
- **`completedAt`:** generated by the finalization transaction; distinct from
  every recipient's `signedAt`, with a biconditional CHECK.
- **Final download:** NOT BUILT. The product explicitly disclaims generating or
  delivering files, so §156's condition is unmet.

## Still open after BACKEND-41

- **Public verification** — BACKEND-42: the projection, the page, rate limiting
  and anti-enumeration.
- **Whether verification supports ID lookup, file-hash check, or both** — a
  product-UX decision BACKEND-42 must make.
- **Final artifact accessibility** — whether a sender, a recipient, or nobody
  may download the completed document, and under what policy.
- **Audit-trail presentation** — BACKEND-43, and its relationship to the
  curated certificate.
- **Completion notifications** — BACKEND-44/45.
- **Future PKI/PAdES/PNPKI extraction** — the triggers are recorded in
  `LAGDA_SEAL_SEMANTICS.md`; none has occurred.
- **⚠ The upload-success/DB-failure window is code-complete but UNTESTED** —
  the fake transaction manager cannot fail mid-transaction. Needs an injectable
  failure or a real-PostgreSQL test. Largest untested path in the completion
  pipeline.
- **⚠ When `email-otp` ships, its challenge store must join the completion
