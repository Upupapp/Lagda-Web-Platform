...
const STATUS_DISPLAY: Record<ServiceStatusValue, { label: string; color: string; bg: string }> = {
  ...
  "not-available":   { label: "Not Available",        color: "#64748B", bg: "rgba(100,116,139,0.1)" },
  ...
  return (
    <ResourcesPageShell>
        ...
        <div style={{ maxWidth: 780, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <p style={{ color: "#94A3B8", ...GM, fontSize: 9, marginBottom: 2 }}>LAST UPDATED</p>
              <p style={{ color: "#64748B", ...GM, fontSize: 11, margin: 0 }}>{MOCK_LAST_UPDATED}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "0 24px 24px" }}>
        <div style={{ background: "rgba(180,83,9,0.06)", border: "1px solid rgba(180,83,9,0.2)", borderRadius: 10, padding: "12px 16px" }}>
          <p style={{ color: "#B45309", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 4 }}>DEMONSTRATION DATA</p>
        ...
        <div style={{ maxWidth: 780, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }} role="list" aria-label="Service status list">
            {MOCK_SERVICES.map(({ id, name, status, note }) => (
              <div key={id} role="listitem" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, padding: "14px 18px", background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 9, flexWrap: "wrap", boxShadow: "0 1px 4px rgba(7,17,31,0.07)" }}>
                <div>
                  <p style={{ color: "#07111F", ...GF, fontSize: 14, fontWeight: 600, margin: 0 }}>{name}</p>
                  {note && <p style={{ color: "#64748B", ...GF, fontSize: 12, margin: "3px 0 0" }}>{note}</p>}
                </div>
      ...
      <ResourcesSection id="incidents" light bordered>
        <div style={{ maxWidth: 780, margin: "0 auto" }}>
          <p style={{ color: "#64748B", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 16 }}>RECENT INCIDENTS</p>
          {MOCK_INCIDENTS.map(({ id, date, title, status: incStatus, detail }) => (
            <div key={id} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "16px 20px", boxShadow: "0 1px 4px rgba(7,17,31,0.07)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap", marginBottom: 8 }}>
                <p style={{ color: "#07111F", ...GF, fontSize: 14, fontWeight: 700, margin: 0 }}>{title}</p>
                <span style={{ color: "#16A34A", ...GM, fontSize: 10, fontWeight: 700, flexShrink: 0 }}>{incStatus.toUpperCase()}</span>
              </div>
              <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: "0 0 8px" }}>{date}</p>
          ...
          <div style={{ display: "grid", gap: 8 }} className="legend-grid">
            {(Object.entries(STATUS_DISPLAY) as [ServiceStatusValue, { label: string; color: string; bg: string }][]).map(([key, { label, color }]) => (
              <div key={key} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ color: "#64748B", ...GF, fontSize: 13 }}><strong style={{ color: "#07111F" }}>{label}</strong> — {key === "operational" ? "System is functioning normally" : key === "degraded" ? "System is slower or partially impaired" : key === "partial-outage" ? "Some features are unavailable" : key === "major-outage" ? "System is unavailable" : key === "maintenance" ? "Planned maintenance in progress" : key === "not-available" ? "Feature is not yet available" : "Future regulated product not yet launched"}</span>
              </div>
            ))}
          </div>
