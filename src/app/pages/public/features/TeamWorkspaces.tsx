...
function WorkspaceMockup() {
  ...
  return (
    <div aria-hidden style={{ background: "#ffffff", border: "1px solid rgba(0,120,212,0.22)", borderRadius: 14, overflow: "hidden", maxWidth: 420, width: "100%" }}>
      <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <p style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 700, margin: 0 }}>Mabini Legal Solutions</p>
          <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: "1px 0 0" }}>Team Workspace</p>
        </div>
        <span style={{ background: "rgba(0,120,212,0.15)", color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 999, border: "1px solid rgba(0,120,212,0.25)" }}>4 members</span>
      </div>
      {members.map((m) => (
        <div key={m.name} style={{ padding: "9px 16px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "rgba(0,120,212,0.2)", display: "flex", alignItems: "center", justifyContent: "center", ...GM, fontSize: 9, fontWeight: 700, color: "#0078D4", flexShrink: 0 }}>{m.avatar}</div>
          <div style={{ flex: 1 }}>
            <p style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 600, margin: 0 }}>{m.name}</p>
            <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: "1px 0 0" }}>{m.role}</p>
          </div>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: m.active ? "#178A4C" : "#64748B", flexShrink: 0 }} />
        </div>
      ))}
      <div style={{ padding: "10px 16px", background: "rgba(0,120,212,0.06)", display: "flex", gap: 12 }}>
        ...
        <span style={{ color: "#64748B", ...GF, fontSize: 12 }}>Manage roles</span>
      </div>
    </div>
  );
}
        ...
        <div style={{ display: "grid", gap: 10 }} className="wr-grid">
          {WORKSPACE_ROLES.map((r) => (
            <div key={r.role} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 14px" }}>
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0, marginBottom: 4 }}>{r.role}</p>
      ...
      <PageSection id="shared" light bordered>
        ...
        <div style={{ display: "grid", gap: 12 }} className="sr-grid">
          {[
            ...
            <div key={item.label} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "14px 14px", display: "flex", gap: 12 }}>
