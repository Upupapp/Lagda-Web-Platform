...
function ContactListMockup() {
  ...
  return (
    <div aria-hidden style={{ background: "#ffffff", border: "1px solid rgba(0,120,212,0.22)", borderRadius: 14, overflow: "hidden", maxWidth: 440, width: "100%" }}>
      <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700 }}>Contacts</span>
        <span style={{ background: "#f8fafb", ...GM, fontSize: 10, color: "#64748B", padding: "2px 8px", borderRadius: 6 }}>Search contacts…</span>
      </div>
      {contacts.map((c) => (
        <div key={c.name} style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(0,120,212,0.2)", display: "flex", alignItems: "center", justifyContent: "center", ...GM, fontSize: 10, fontWeight: 700, color: "#0078D4", flexShrink: 0 }}>{c.avatar}</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 600, margin: 0 }}>{c.name}</p>
            <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: "1px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.role} · {c.org}</p>
          </div>
          <span style={{ background: "rgba(0,120,212,0.1)", color: "#0078D4", border: "1px solid rgba(0,120,212,0.2)", ...GM, fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 999, flexShrink: 0 }}>{c.tag}</span>
        </div>
      ))}
    ...
    <FeaturesPageShell>
            ...
            <div style={{ display: "grid", gap: 6 }} className="con-info-grid">
              {CONTACT_INFO.map((f) => (
                <div key={f} style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
                  ...
                  <span style={{ color: "#334155", ...GF, fontSize: 13, lineHeight: 1.45 }}>{f}</span>
                </div>
              ))}
            </div>
      ...
      <PageSection id="actions">
        ...
        <div style={{ display: "grid", gap: 10 }} className="ca-grid">
          {[
            ...
            <div key={a.action} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 12px" }}>
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0, marginBottom: 4 }}>{a.action}</p>
      ...
      <PageSection id="crm-note" light bordered>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <p style={{ color: "#9A7208", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 10 }}>CONTACTS ARE NOT A CRM</p>
