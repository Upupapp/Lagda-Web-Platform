...
function TemplateLibraryMockup() {
  ...
  return (
    <div aria-hidden style={{ background: "#ffffff", border: "1px solid rgba(0,120,212,0.22)", borderRadius: 14, overflow: "hidden", maxWidth: 460, width: "100%" }}>
      <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700 }}>Template Library</span>
        <span style={{ background: "rgba(0,120,212,0.15)", color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700, padding: "2px 10px", borderRadius: 999, border: "1px solid rgba(0,120,212,0.25)" }}>4 templates</span>
      </div>
      {templates.map((t) => (
        <div key={t.name} style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 600, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</p>
            <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: "2px 0 0" }}>{t.roles} roles · {t.fields} fields · {t.uses}× used</p>
          </div>
          <span style={{
            background: t.status === "Ready" ? "rgba(34,197,94,0.1)" : "rgba(100,116,139,0.1)",
            color: t.status === "Ready" ? "#178A4C" : "#334155",
...
const TEMPLATE_STATUSES = [
  { status: "Ready",            color: "#178A4C", desc: "Complete and available to authorized senders in the workspace." },
  ...
  { status: "Restricted",       color: "#334155", desc: "Available only to specific roles or senders." },
  { status: "Archived",         color: "#64748B", desc: "No longer active. Existing transactions are retained." },
      ...
      <PageSection id="library" light bordered>
        <div style={{ display: "grid", gap: "32px 48px", alignItems: "start" }} className="tmpl-two-col">
          <div>
            ...
            <p style={{ color: "#64748B", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 10 }}>WHAT A TEMPLATE RETAINS</p>
            <div style={{ display: "grid", gap: 6 }} className="tf-grid">
              {TEMPLATE_FIELDS.map((f) => (
                <div key={f} style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
                  ...
                  <span style={{ color: "#334155", ...GF, fontSize: 12, lineHeight: 1.45 }}>{f}</span>
                </div>
              ))}
            </div>
            <style>{`.tf-grid { grid-template-columns: 1fr 1fr; } @media (max-width: 480px) { .tf-grid { grid-template-columns: 1fr; } }`}</style>
            <p style={{ color: "#64748B", ...GF, fontSize: 13, marginTop: 14, lineHeight: 1.6 }}>
      ...
      <PageSection id="roles">
        ...
        <div style={{ display: "grid", gap: 10 }} className="rp-grid">
          {["Client", "Lawyer / Counsel", "Authorized Representative", "HR Manager", "Finance Approver", "Copy Recipient"].map((r) => (
            <div key={r} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 12px" }}>
              <span style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700 }}>ROLE</span>
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 600, margin: "4px 0 0" }}>{r}</p>
            </div>
          ))}
        </div>
      ...
      <PageSection id="statuses" light bordered>
        ...
        <div style={{ display: "grid", gap: 10 }} className="ts-grid">
          {TEMPLATE_STATUSES.map((s) => (
            <div key={s.status} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 12px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: s.color }} />
                <span style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700 }}>{s.status}</span>
              </div>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <span style={{ color: "#64748B", ...GF, fontSize: 13, marginLeft: 10 }}>Template availability and sharing controls may be plan-dependent.</span>
        </div>
