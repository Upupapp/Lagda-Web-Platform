...
function DocWorkspaceMockup() {
  ...
  return (
    <div aria-hidden style={{ background: "#ffffff", border: "1px solid rgba(0,120,212,0.22)", borderRadius: 14, overflow: "hidden", maxWidth: 480, width: "100%" }}>
      {/* Toolbar */}
      <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", gap: 8, flexWrap: "wrap" }}>
        {["Signature", "Initials", "Date", "Text", "Checkbox"].map((t) => (
          <span key={t} style={{ background: "rgba(0,120,212,0.12)", border: "1px solid rgba(0,120,212,0.25)", color: "#0078D4", ...GM, fontSize: 9, fontWeight: 700, padding: "3px 8px", borderRadius: 6 }}>{t}</span>
        ))}
      </div>
      {/* Document preview */}
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10 }}>
          <div style={{ width: 32, height: 40, background: "#f1f5f9", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", ...GM, fontSize: 8, color: "#64748B" }}>PDF</div>
          <div>
            <p style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 600, margin: 0 }}>Professional Services Agreement</p>
            <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: 0 }}>3 pages · 4 fields placed</p>
          </div>
        </div>
        <div style={{ display: "flex", gap: 4 }}>
          {[1, 2, 3].map((pg) => (
            <div key={pg} style={{ flex: 1, height: 48, background: pg === 3 ? "rgba(0,120,212,0.08)" : "#f8fafb", border: `1px solid ${pg === 3 ? "rgba(0,120,212,0.3)" : "rgba(0,0,0,0.08)"}`, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", ...GM, fontSize: 9, color: "#64748B" }}>
              Pg {pg}{pg === 3 && " ●"}
            </div>
          ))}
        </div>
      ...
      <div style={{ padding: "8px 0" }}>
        {fields.map((f) => (
          <div key={f.type + f.participant} style={{ padding: "7px 16px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ background: "rgba(0,120,212,0.12)", color: "#0078D4", ...GM, fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, flexShrink: 0 }}>{f.type}</span>
              <span style={{ color: "#64748B", ...GF, fontSize: 11 }}>{f.participant}</span>
            </div>
            <span style={{ color: "#64748B", ...GM, fontSize: 9 }}>pg {f.page}</span>
          </div>
        ))}
      </div>
      {/* Summary */}
      <div style={{ padding: "10px 16px", background: "rgba(0,120,212,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ color: "#334155", ...GF, fontSize: 12 }}>Ready to review</span>
    ...
    <FeaturesPageShell>
      ...
      <PageSection id="field-types">
        ...
        <div style={{ display: "grid", gap: 10 }} className="ft-grid">
          {FIELD_TYPES.map((f) => (
            <div key={f.type} style={{ background: "#f8fafb", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 14px" }}>
              <p style={{ color: "#0078D4", ...GM, fontSize: 11, fontWeight: 700, margin: 0, marginBottom: 4 }}>{f.type}</p>
        ...
        <div style={{ display: "grid", gap: "32px 48px", alignItems: "center" }} className="dp-two-col">
          <div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <span style={{ background: "rgba(0,120,212,0.08)", border: "1px solid rgba(0,120,212,0.2)", color: "#0078D4", ...GM, fontSize: 11, fontWeight: 700, padding: "6px 12px", borderRadius: 8 }}>Templates →</span>
            </div>
          </div>
          <div style={{ background: "rgba(0,120,212,0.06)", border: "1px solid rgba(0,120,212,0.15)", borderRadius: 14, padding: "20px 18px" }}>
            <p style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 10 }}>TEMPLATE REUSE</p>
      ...
      <PageSection id="prep-note">
        <div style={{ background: "#f8fafb", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 14, padding: "20px 24px" }}>
          <p style={{ color: "#334155", ...GF, fontSize: 14, lineHeight: 1.65, margin: 0 }}>
            <strong style={{ color: "#07111F" }}>Note:</strong> The document-preparation interface is part of the authenticated LAGDA platform. The example above is illustrative. File-size and page limits vary by plan.
          </p>
        </div>
