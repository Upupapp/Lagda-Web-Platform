...
function Field({ id, label, children, error, required, optional }: { id: string; label: string; children: React.ReactNode; error?: string; required?: boolean; optional?: boolean }) {
  return (
    <div>
      <label htmlFor={id} style={{ display: "block", color: "#334155", ...GF, fontSize: 12, fontWeight: 600, marginBottom: 5 }}>
        {label}
        {required && <span aria-hidden style={{ color: "#DC2626" }}> *</span>}
        {optional && <span style={{ color: "#94A3B8" }}> (optional)</span>}
      </label>
      {children}
      {error && <p id={`${id}-err`} role="alert" style={{ color: "#DC2626", ...GF, fontSize: 12, margin: "4px 0 0" }}>{error}</p>}
    </div>
  );
}

const inputStyle: React.CSSProperties = { width: "100%", boxSizing: "border-box", background: "#ffffff", border: "1px solid rgba(0,0,0,0.14)", borderRadius: 8, color: "#07111F", fontFamily: "'Geist', sans-serif", fontSize: 14, padding: "11px 14px", outline: "none" };
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "80px 24px", textAlign: "center" }}>
          <div ref={confirmRef} tabIndex={-1} style={{ outline: "none" }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <Link to="/security/trust-center" style={{ background: "#ffffff", color: "#07111F", ...GF, fontSize: 14, fontWeight: 600, padding: "12px 22px", borderRadius: 8, textDecoration: "none", border: "1px solid rgba(0,0,0,0.14)" }}>Visit Trust Center</Link>
            </div>
    ...
    <div style={{ background: "#ffffff", minHeight: "100vh", color: "#07111F", ...GF }}>
      <section style={{ padding: "64px 24px 48px", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          ...
          {parsedSolution && (
            <div style={{ marginTop: 16, display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(0,120,212,0.06)", border: "1px solid rgba(0,120,212,0.2)", borderRadius: 6, padding: "5px 12px" }}>
              <span style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700 }}>CONTEXT</span>
              <span style={{ color: "#07111F", ...GF, fontSize: 12 }}>{SOLUTION_DISPLAY_NAMES[parsedSolution]}</span>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        <div style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 8, padding: "11px 14px", marginBottom: 24, boxShadow: "0 1px 4px rgba(7,17,31,0.07)" }}>
          <p style={{ color: "#64748B", ...GF, fontSize: 12, lineHeight: 1.55, margin: 0 }}>
        ...
        {serverError && (
          <div ref={errorRef} tabIndex={-1} role="alert" style={{ background: "rgba(220,38,38,0.06)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 8, padding: "12px 14px", marginBottom: 18, outline: "none" }}>
            ...
            <button onClick={() => { setServerError(null); setStatus("idle"); }} style={{ color: "#0078D4", ...GF, fontSize: 12, background: "none", border: "none", cursor: "pointer", padding: "4px 0 0" }}>Try again</button>
          </div>
        )}
        {Object.keys(errors).length > 0 && !serverError && (
          <div ref={errorRef} tabIndex={-1} role="alert" style={{ background: "rgba(220,38,38,0.06)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 8, padding: "12px 14px", marginBottom: 18, outline: "none" }}>
            <p style={{ color: "#DC2626", ...GF, fontSize: 13, fontWeight: 600, margin: "0 0 6px" }}>Please correct the following:</p>
            <ul style={{ margin: 0, padding: "0 0 0 16px" }}>
              {Object.values(errors).map((m) => <li key={m} style={{ color: "#DC2626", ...GF, fontSize: 12, lineHeight: 1.5 }}>{m}</li>)}
            </ul>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} noValidate aria-label="Book a demo form" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
            <Field id="demo-name" label="Full name" error={errors.name} required>
              <input id="demo-name" type="text" value={fields.name} onChange={(e) => set("name", e.target.value)} autoComplete="name" aria-required aria-invalid={!!errors.name} aria-describedby={errors.name ? "demo-name-err" : undefined} style={{ ...inputStyle, borderColor: errors.name ? "rgba(220,38,38,0.4)" : "rgba(0,0,0,0.14)" }} />
            </Field>
            <Field id="demo-email" label="Work email" error={errors.email} required>
              <input id="demo-email" type="email" value={fields.email} onChange={(e) => set("email", e.target.value)} autoComplete="email" aria-required aria-invalid={!!errors.email} aria-describedby={errors.email ? "demo-email-err" : undefined} style={{ ...inputStyle, borderColor: errors.email ? "rgba(220,38,38,0.4)" : "rgba(0,0,0,0.14)" }} />
            </Field>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
            <Field id="demo-org" label="Organization" error={errors.organization} required>
              <input id="demo-org" type="text" value={fields.organization} onChange={(e) => set("organization", e.target.value)} autoComplete="organization" aria-required aria-invalid={!!errors.organization} aria-describedby={errors.organization ? "demo-org-err" : undefined} style={{ ...inputStyle, borderColor: errors.organization ? "rgba(220,38,38,0.4)" : "rgba(0,0,0,0.14)" }} />
            </Field>
            <Field id="demo-role" label="Your role" optional>
              <select id="demo-role" value={fields.role} onChange={(e) => set("role", e.target.value as DemoRequest["role"])} style={{ ...inputStyle, cursor: "pointer" }}>
                ...
                {(Object.entries(INQUIRY_ROLE_LABELS)).map(([v, l]) => <option key={v} value={v} style={{ background: "#ffffff" }}>{l}</option>)}
              </select>
            </Field>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
            <Field id="demo-size" label="Organization size" optional>
              <select id="demo-size" value={fields.orgSize} onChange={(e) => set("orgSize", e.target.value as DemoRequest["orgSize"])} style={{ ...inputStyle, cursor: "pointer" }}>
                <option value="" style={{ background: "#ffffff" }}>Select…</option>
                {(Object.entries(ORG_SIZE_LABELS)).map(([v, l]) => <option key={v} value={v} style={{ background: "#ffffff" }}>{l}</option>)}
              </select>
            </Field>
            <Field id="demo-industry" label="Industry" optional>
              <select id="demo-industry" value={fields.industry} onChange={(e) => set("industry", e.target.value)} style={{ ...inputStyle, cursor: "pointer" }}>
                {INDUSTRY_OPTIONS.map((o) => <option key={o} value={o} style={{ background: "#ffffff" }}>{o || "Select…"}</option>)}
              </select>
            </Field>
          </div>
          ...
          <Field id="demo-interest" label="Primary interest" error={errors.primaryInterest} required>
            <select id="demo-interest" value={fields.primaryInterest} onChange={(e) => set("primaryInterest", e.target.value as DemoTopic | "")} aria-required aria-invalid={!!errors.primaryInterest} aria-describedby={errors.primaryInterest ? "demo-interest-err" : undefined} style={{ ...inputStyle, cursor: "pointer", borderColor: errors.primaryInterest ? "rgba(220,38,38,0.4)" : "rgba(0,0,0,0.14)" }}>
              <option value="" style={{ background: "#ffffff" }}>Select your primary interest…</option>
              {VALID_DEMO_TOPICS.map((t) => <option key={t} value={t} style={{ background: "#ffffff" }}>{DEMO_TOPIC_LABELS[t]}</option>)}
            </select>
          </Field>

          {isEnotaryInterest && (
            <div style={{ background: "rgba(103,2,59,0.06)", border: "1px solid rgba(103,2,59,0.25)", borderRadius: 9, padding: "12px 16px" }}>
              <p style={{ color: "#334155", ...GF, fontSize: 13, lineHeight: 1.65, margin: 0 }}>
          ...
          <div style={{ background: "#ffffff", border: errors.consent ? "1px solid rgba(220,38,38,0.3)" : "1px solid rgba(0,0,0,0.1)", borderRadius: 8, padding: "12px 14px" }}>
        ...
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: "1px solid rgba(0,0,0,0.06)" }}>
          <p style={{ color: "#64748B", ...GF, fontSize: 13, lineHeight: 1.65, margin: "0 0 10px" }}>
            Looking for something else?
          </p>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Link to="/contact?category=sales" style={{ color: "#0078D4", ...GF, fontSize: 13, textDecoration: "none" }}>Contact Sales →</Link>
