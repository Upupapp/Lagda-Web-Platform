...
function GovernmentWorkflowMockup() {
  const flow = [
    ...
    { label: "Department Reviewer",     action: "In review",         status: "ACTIVE",  color: "#7C3AED" },
    { label: "Finance — Budget Office", action: "Awaiting",          status: "PENDING", color: "#94A3B8" },
    { label: "Authorized Officer",      action: "Awaiting",          status: "PENDING", color: "#94A3B8" },
  ];
  return (
    <div aria-hidden style={{
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(0,0,0,0.08)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <p style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 700, margin: 0 }}>Purchase Request for Approval</p>
          <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: "1px 0 0" }}>Internal workflow · Office document routing</p>
        </div>
        <span style={{ background: "rgba(124,58,237,0.1)", color: "#7C3AED", ...GM, fontSize: 9, fontWeight: 700, padding: "2px 8px", borderRadius: 999, border: "1px solid rgba(124,58,237,0.25)" }}>IN REVIEW</span>
      </div>
      {flow.map((s, i) => (
        <div key={i} style={{ padding: "9px 16px", borderBottom: i < flow.length - 1 ? "1px solid rgba(0,0,0,0.06)" : "none", display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{
            width: 28, height: 28, borderRadius: "50%",
            background: s.status === "DONE" ? "rgba(34,197,94,0.12)" : s.status === "ACTIVE" ? "rgba(124,58,237,0.12)" : "rgba(0,0,0,0.04)",
      ...
      <div style={{ padding: "10px 16px", background: "#f8fafb" }}>
        <span style={{ color: "#94A3B8", ...GM, fontSize: 9 }}>Subject to agency policy and applicable procurement rules</span>
      </div>
    </div>
  );
}
    ...
    <SolPageShell>
      ...
      <PageSection id="challenges">
        <div style={{ display: "grid", gap: "32px 48px", alignItems: "start" }} className="gov-two-col">
          ...
          <div>
            <p style={{ color: "#64748B", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 12 }}>APPROVAL WORKFLOW EXAMPLE</p>
