...
function ProgressTracker() {
  const STATUSES = [
    ...
    { label: "Ana Reyes — Signed", color: "#178A4C", done: true },
  ...
  return (
    <div aria-hidden style={{
      ...
      <p style={{ color: "#64748B", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", marginBottom: 16 }}>
        ...
        <div key={s.label} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: i < STATUSES.length - 1 ? 0 : 0 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
            <div style={{
              ...
              background: s.done ? s.color : "rgba(0,0,0,0.08)",
            ...
            {i < STATUSES.length - 1 && (
              <div style={{ width: 1, height: 20, background: s.done ? "rgba(0,120,212,0.3)" : "rgba(0,0,0,0.08)", margin: "2px 0" }} />
            )}
          </div>
          <div style={{ paddingBottom: i < STATUSES.length - 1 ? 16 : 0 }}>
            <span style={{ color: s.done ? "#07111F" : "#94A3B8", ...GF, fontSize: 12, fontWeight: s.done ? 600 : 400 }}>
              {s.label}
            </span>
          </div>
        </div>
  ...
  return (
    <div aria-hidden style={{
      ...
      <div style={{ padding: "12px 18px", borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <span style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700 }}>{AUDIT_EVENTS.length} EVENTS</span>
      </div>
      <div style={{ padding: "6px 0" }}>
        {AUDIT_EVENTS.map((ev, i) => (
          <div key={i} style={{ padding: "9px 18px", display: "flex", gap: 10, alignItems: "flex-start", borderBottom: i < AUDIT_EVENTS.length - 1 ? "1px solid rgba(0,0,0,0.05)" : "none" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ color: "#64748B", ...GM, fontSize: 10, margin: "2px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.who}</p>
            </div>
        ...
        {VERIFICATION_STATES.map((s) => (
          <button key={s.id} onClick={() => setActiveId(s.id)}
            ...
            style={{
              ...
              background: s.id === activeId ? "rgba(0,120,212,0.1)" : "#f8fafb",
              border: `1px solid ${s.id === activeId ? "rgba(0,120,212,0.35)" : "rgba(0,0,0,0.08)"}`,
              color: s.id === activeId ? "#0078D4" : "#64748B",
      ...
      <div style={{
        ...
        <div style={{ padding: "8px 0" }}>
          {DETAILS[active.id]?.map((f) => (
            <div key={f.label} style={{ padding: "8px 18px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", gap: 12, alignItems: "flex-start" }}>
              <span style={{ color: "#64748B", ...GM, fontSize: 10, flexShrink: 0, minWidth: 80, paddingTop: 2 }}>{f.label}</span>
              <span style={{ color: "#07111F", ...(f.mono ? GM : GF), fontSize: 12, lineHeight: 1.4, wordBreak: "break-word" }}>{f.value}</span>
            </div>
          ))}
        </div>
        <div style={{ padding: "10px 18px", background: "#f8fafb" }}>
          <p style={{ color: "#94A3B8", ...GF, fontSize: 11, margin: 0, lineHeight: 1.5 }}>
    ...
    <PageSection id="public-private" light bordered>
      ...
      <div style={{ display: "grid", gap: 20 }} className="pp-grid">
        <div style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 14, padding: "20px 20px" }}>
          <p style={{ color: "#178A4C", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 14 }}>PUBLICLY SHOWN BY DEFAULT</p>
          {PUBLIC.map((item) => (
            <div key={item} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 8 }}>
              <span style={{ color: "#178A4C", fontWeight: 700, flexShrink: 0, fontSize: 13 }}>✓</span>
      ...
      <PageSection id="evidence" light bordered>
        ...
        <div style={{ display: "grid", gap: 12 }} className="ev-grid">
          {[
            ...
            <div key={e.label} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 14px", boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0, marginBottom: 4 }}>{e.label}</p>
              <p style={{ color: "#64748B", ...GF, fontSize: 12, margin: 0, lineHeight: 1.5 }}>{e.desc}</p>
            </div>
          ))}
        </div>
        <p style={{ color: "#64748B", ...GF, fontSize: 13, lineHeight: 1.6, margin: "20px 0 0" }}>
      ...
      <PageSection id="verification">
        <div style={{ display: "grid", gap: "32px 48px", alignItems: "start" }} className="va-two-col">
          <div>
            ...
            <p style={{ color: "#64748B", ...GF, fontSize: 13, lineHeight: 1.6, margin: 0 }}>
              <strong style={{ color: "#334155" }}>Verification methods:</strong> QR code, Verification ID, secure verification link, or document upload where supported.
            </p>
          </div>
          <VerificationDemo />
        </div>
