...
function TryItNowPanel() {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 16 }}>
      ...
      />
      <p style={{ color: AZURE, ...GF, fontSize: 15, fontWeight: 800, margin: 0 }}>
        Try It Now!
      </p>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        ...
        >
          ...
          <span aria-hidden style={{
            ...
            width: 26, height: 26, borderRadius: "50%", background: "rgba(255,255,255,0.22)",
          }}>
...
function DocumentSetupMockup() {
  return (
    <div aria-hidden style={{
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ fontSize: 14 }}>📄</span>
        <span style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 700 }}>Professional Services Agreement.pdf</span>
      </div>
      <div style={{ padding: "14px 16px" }}>
        <p style={{ color: "#64748B", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", marginBottom: 10 }}>FIELDS PLACED</p>
        {[
          ...
          <div key={f.field} style={{
            ...
            <span style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 600, flex: 1 }}>{f.field}</span>
            <span style={{ color: "#64748B", ...GM, fontSize: 10 }}>{f.by}</span>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
        ...
        {(["sequential", "parallel"] as const).map((m) => (
          <button key={m} onClick={() => setMode(m)}
            ...
            style={{
              ...
              background: mode === m ? "rgba(0,120,212,0.1)" : "#f8fafb",
              border: `1px solid ${mode === m ? "rgba(0,120,212,0.35)" : "rgba(0,0,0,0.08)"}`,
              color: mode === m ? "#0078D4" : "#64748B",
      ...
      <div style={{
        ...
        {mode === "sequential" ? (
          <div>
            <p style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 16 }}>SEQUENTIAL — ACTS IN ORDER</p>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              {["Ana Reyes · Signer", "→", "Marco Santos · Approver", "→", "Completed"].map((item, i) => (
                item === "→" ? (
                  ...
                  <div key={i} style={{
                    background: item === "Completed" ? "rgba(34,197,94,0.1)" : "rgba(0,120,212,0.08)",
                    border: `1px solid ${item === "Completed" ? "rgba(34,197,94,0.25)" : "rgba(0,120,212,0.22)"}`,
                    borderRadius: 8, padding: "7px 12px",
                    color: item === "Completed" ? "#178A4C" : "#07111F",
            ...
            <p style={{ color: "#64748B", ...GF, fontSize: 13, lineHeight: 1.5, margin: "16px 0 0" }}>
              Each participant acts only after the previous one completes. Useful when an approval or authority depends on prior action.
            </p>
          </div>
        ) : (
          <div>
            <p style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 16 }}>PARALLEL — ACT SIMULTANEOUSLY</p>
            <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
              {["Ana Reyes · Signer", "Marco Santos · Signer"].map((p) => (
                <div key={p} style={{
                  ...
                  borderRadius: 8, padding: "7px 12px",
                  color: "#07111F", ...GF, fontSize: 12, fontWeight: 600,
                }}>
                  {p}
                </div>
              ))}
            ...
            <div style={{ background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 8, padding: "7px 12px", display: "inline-flex", color: "#178A4C", ...GF, fontSize: 12, fontWeight: 600 }}>
  ...
  return (
    <div aria-hidden style={{
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
        <span style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700 }}>Transaction Summary</span>
      </div>
      {[
        ...
        <div key={row.label} style={{ padding: "9px 16px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", gap: 12, alignItems: "flex-start" }}>
          <span style={{ color: "#64748B", ...GM, fontSize: 10, flexShrink: 0, paddingTop: 2, minWidth: 80 }}>{row.label}</span>
          <span style={{ color: "#334155", ...GF, fontSize: 12, lineHeight: 1.4 }}>{row.value}</span>
        </div>
      ))}
      <div style={{ padding: "12px 16px", background: "rgba(0,120,212,0.06)" }}>
  ...
  return (
    <EsigPageShell>
      <PageHero
        ...
        visual={<TryItNowPanel />}
      >
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 24 }}>
          <a href="#prepare" style={{ color: "#0078D4", ...GF, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>Prepare ↓</a>
      ...
      <PageSection id="participants">
        ...
        <div style={{ display: "grid", gap: 12 }} className="roles-grid">
          {PARTICIPANT_ROLES.map((r) => (
            <div key={r.label} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 12, padding: "14px 16px", boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0, marginBottom: 4 }}>{r.label}</p>
              <p style={{ color: "#64748B", ...GF, fontSize: 13, margin: 0, lineHeight: 1.5 }}>{r.desc}</p>
            </div>
          ))}
        </div>
        <p style={{ color: "#94A3B8", ...GF, fontSize: 12, lineHeight: 1.6, marginTop: 16 }}>
      ...
      <PageSection id="routing" light bordered>
        <div style={{ display: "grid", gap: "32px 48px", alignItems: "start" }} className="cw-two-col">
          <div>
            ...
            <div style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 12, padding: "14px 16px", marginTop: 16, boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
              <p style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 6 }}>EXAMPLE — SEQUENTIAL</p>
      ...
      <PageSection id="authentication">
        ...
        <div style={{ display: "grid", gap: 10 }} className="auth-grid">
          {AUTH_METHODS.map((m) => (
            <div key={m.label} style={{ display: "flex", gap: 12, alignItems: "flex-start", background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 14px", boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
              <span style={{ color: m.available ? "#178A4C" : "#9A7208", fontWeight: 700, flexShrink: 0, fontSize: 13 }}>{m.available ? "✓" : "★"}</span>
              <div>
                <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0, marginBottom: 2 }}>
                  {m.label}
                  {!m.available && <span style={{ color: "#9A7208", ...GM, fontSize: 9, fontWeight: 700, marginLeft: 8 }}>ENTERPRISE</span>}
                </p>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
              {FIELD_TYPES.map((f) => (
                <div key={f.label} style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "8px 0" }}>
                  ...
                  <div>
                    <p style={{ color: "#07111F", ...GF, fontSize: 12, fontWeight: 700, margin: 0 }}>{f.label}</p>
      ...
      <PageSection id="recipient">
        ...
        <div style={{ display: "grid", gap: 10 }} className="recip-grid">
          {RECIPIENT_STEPS.map((s) => (
            <div key={s.num} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 12, padding: "14px 14px", boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
