    ...
    <SecurityPageShell>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 620 }}>
            ...
            <tbody>
              {AUTH_COMPARISON.map((row) => (
                <tr key={row.method} style={{ borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
                  <td style={{ padding: "10px 12px", color: "#07111F", ...GF, fontSize: 12, fontWeight: 600 }}>{row.method}</td>
                  <td style={{ padding: "10px 12px", color: "#334155", ...GF, fontSize: 12 }}>{row.experience}</td>
                  <td style={{ padding: "10px 12px", textAlign: "center" }}>
                    <span style={{ color: row.independentChannel ? "#16A34A" : "#94A3B8", fontSize: 14 }}>{row.independentChannel ? "✓" : "—"}</span>
                  </td>
                  <td style={{ padding: "10px 12px" }}>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {row.evidenceRecorded.map((e) => (
                        <span key={e} style={{ background: "rgba(0,0,0,0.04)", color: "#64748B", ...GM, fontSize: 9, padding: "2px 6px", borderRadius: 4 }}>{e}</span>
                      ))}
                    </div>
                  </td>
                  <td style={{ padding: "10px 12px" }}>
      ...
      <PageSection id="channel-concept">
        ...
        <div style={{ display: "grid", gap: 12 }} className="ic-grid">
          <div style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 12, padding: "16px", boxShadow: "0 1px 4px rgba(7,17,31,0.07)" }}>
            <p style={{ color: "#64748B", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", marginBottom: 10 }}>SAME-CHANNEL (Email OTP)</p>
        ...
        <div style={{ display: "grid", gap: 10 }} className="rm-grid">
          {[
            ...
            <div key={r.tier} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "14px 14px", boxShadow: "0 1px 4px rgba(7,17,31,0.07)" }}>
              ...
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0, marginBottom: 4 }}>{r.method}</p>
              <p style={{ color: "#64748B", ...GF, fontSize: 12, lineHeight: 1.5, margin: 0 }}>{r.examples}</p>
            </div>
          ))}
        </div>
        <p style={{ color: "#64748B", ...GF, fontSize: 13, marginTop: 14, lineHeight: 1.6, textAlign: "center" }}>
