...
const STATUS_LEGEND = [
  { color: "#16A34A",  label: "In progress",            desc: "Currently underway" },
  { color: "#0078D4",  label: "Planned",                 desc: "Scheduled for a future stage" },
  { color: BURGUNDY,   label: "Future regulatory step",  desc: "Depends on regulatory process" },
  { color: "#94A3B8",  label: "After required approval", desc: "Follows accreditation and launch" },
];
    ...
    <EnotaryPageShell>
        ...
        <div style={{ maxWidth: 920, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
            {WHY_ACCREDITATION.map(({ heading, body }) => (
              <div key={heading} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "18px 20px", boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
                <p style={{ color: "#07111F", ...GF, fontSize: 14, fontWeight: 700, margin: "0 0 8px" }}>{heading}</p>
      ...
      <EnotarySection id="timeline" light bordered>
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          ...
          {/* Legend */}
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 32, padding: "12px 16px", background: "#ffffff", borderRadius: 8, border: "1px solid rgba(0,0,0,0.08)" }}>
            {STATUS_LEGEND.map(({ color, label, desc }) => (
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ width: 10, height: 10, borderRadius: "50%", background: color, display: "inline-block", flexShrink: 0 }} />
                <span style={{ color: "#64748B", ...GF, fontSize: 12 }}><strong style={{ color: "#07111F" }}>{label}</strong> — {desc}</span>
              </div>
            ))}
          </div>
        ...
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              ...
              <div key={note} style={{ display: "flex", gap: 12, padding: "12px 16px", background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 8 }}>
                <span style={{ color: "#94A3B8", flexShrink: 0 }}>—</span>
                <span style={{ color: "#64748B", ...GF, fontSize: 13, lineHeight: 1.65 }}>{note}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <EnotarySection id="updates" light bordered>
        <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ color: "#07111F", ...GF, fontSize: "clamp(18px, 2.5vw, 26px)", fontWeight: 900, margin: "0 0 12px" }}>Receive updates</h2>
