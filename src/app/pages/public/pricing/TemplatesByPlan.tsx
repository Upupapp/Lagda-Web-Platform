    ...
    <PricingPageShell>
      <section style={{ padding: "64px 24px 48px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <p style={{ color: "#0078D4", ...GM, fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 12 }}>TEMPLATES BY PLAN</p>
      ...
      <PricingSection id="what-is-template" light bordered>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <p style={{ color: "#64748B", ...GF, fontSize: 14, lineHeight: 1.65 }}>
              ...
              {TEMPLATE_DIMENSIONS.map(({ icon, title, personal, business, enterprise }) => (
                <tr key={title} style={{ borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                  ...
                  <td style={{ textAlign: "center", padding: "10px 12px", background: "rgba(0,120,212,0.04)" }}>
                    {business ? <span style={{ color: "#16A34A" }}>✓</span> : <span style={{ color: "#94A3B8" }}>—</span>}
                  </td>
                  <td style={{ textAlign: "center", padding: "10px 12px" }}>
                    {enterprise ? <span style={{ color: "#16A34A" }}>✓</span> : <span style={{ color: "#94A3B8" }}>—</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <PricingSection id="examples" light bordered>
        ...
        <div style={{ display: "grid", gap: 12, maxWidth: 900, margin: "0 auto" }} className="ex-grid">
          {TEMPLATE_USES.map(({ title, desc }) => (
            <div key={title} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "14px 18px", boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
      ...
      <PricingSection id="cta">
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h2 style={{ color: "#07111F", ...GF, fontSize: 24, fontWeight: 800, margin: "0 0 8px" }}>Explore templates in the product.</h2>
            <p style={{ color: "#64748B", ...GF, fontSize: 14, margin: 0 }}>See how templates are built, shared, and managed across your workspace.</p>
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <Link to="/pricing/compare" style={{ background: "#ffffff", color: "#07111F", ...GF, fontSize: 14, fontWeight: 600, padding: "11px 24px", borderRadius: 8, textDecoration: "none", minHeight: 44, display: "flex", alignItems: "center", border: "1px solid rgba(0,0,0,0.14)" }}>Compare Plans</Link>
          </div>
        </div>
      </PricingSection>
    </PricingPageShell>
