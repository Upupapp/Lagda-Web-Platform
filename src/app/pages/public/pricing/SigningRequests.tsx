...
function SigningRequestMockup() {
  ...
  return (
    <div aria-hidden style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 14, overflow: "hidden", maxWidth: 440, width: "100%", boxShadow: "0 4px 16px rgba(7,17,31,0.10), 0 1px 4px rgba(7,17,31,0.05)" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(0,0,0,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <p style={{ color: "#64748B", ...GM, fontSize: 9, margin: 0 }}>SENT THIS PERIOD</p>
        </div>
      </div>
      {txns.map((t, i) => (
        <div key={t.id} style={{ padding: "10px 16px", borderBottom: i < txns.length - 1 ? "1px solid rgba(0,0,0,0.06)" : "none", display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ color: t.status === "DRAFT" ? "#94A3B8" : "#07111F", ...GF, fontSize: 12, fontWeight: 600, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.title}</p>
            <p style={{ color: "#94A3B8", ...GM, fontSize: 9, margin: "2px 0 0" }}>{t.date}</p>
          </div>
          <span style={{ color: t.status === "COMPLETED" ? "#16A34A" : t.status === "DRAFT" ? "#94A3B8" : "#0078D4", ...GM, fontSize: 9, fontWeight: 700, flexShrink: 0 }}>{t.status}</span>
          <span style={{ color: t.counted ? "#64748B" : "#94A3B8", ...GM, fontSize: 9, flexShrink: 0 }}>{t.counted ? "COUNTED" : "NOT COUNTED"}</span>
        </div>
      ))}
      <div style={{ padding: "10px 16px", background: "#f8fafb", display: "flex", justifyContent: "space-between" }}>
        <span style={{ color: "#94A3B8", ...GM, fontSize: 9 }}>Drafts are not counted until sent</span>
        <span style={{ color: "#0078D4", ...GM, fontSize: 9, fontWeight: 700 }}>3 REQUESTS SENT</span>
      </div>
    </div>
  );
}
    ...
    <PricingPageShell>
      <section style={{ padding: "64px 24px 48px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <p style={{ color: "#0078D4", ...GM, fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 12 }}>SIGNING REQUESTS</p>
          ...
          <div>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {SIGNING_REQUEST_NOTES.map(({ icon, title, body }) => (
                <div key={title} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 12, padding: "16px 20px", display: "flex", gap: 14, boxShadow: "0 1px 4px rgba(7,17,31,0.07)" }}>
