...
const SOFT_BURGUNDY = "rgba(103,2,59,0.06)";
  ...
  return (
    <div style={{ background: SOFT_BURGUNDY, borderBottom: `1px solid rgba(103,2,59,0.25)`, padding: "12px 24px" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <p style={{ color: "#67023B", ...GF, fontSize: 13, lineHeight: 1.5, margin: 0 }}>
          {ENOTARY_DISCLAIMER}
        </p>
      </div>
    </div>
...
export function EnotaryDisclaimer({ variant = "default" }: { variant?: "default" | "compact" }) {
  if (variant === "compact") {
    return (
      <p style={{ color: "#64748B", ...GF, fontSize: 12, lineHeight: 1.6, margin: "12px 0 0" }}>
        {ENOTARY_DISCLAIMER}
      </p>
    );
  }
...
export function EnotaryHeading({ children, sub }: { children: React.ReactNode; sub?: string }) {
  return (
    <div style={{ marginBottom: 36 }}>
      <h2 style={{ color: "#07111F", ...GF, fontSize: "clamp(20px, 3vw, 30px)", fontWeight: 900, lineHeight: 1.15, letterSpacing: "-0.02em", margin: "0 0 10px" }}>{children}</h2>
...
export function FutureConceptCard({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ background: "rgba(103,2,59,0.05)", border: `1px solid rgba(176,18,98,0.2)`, borderRadius: 12, padding: "20px 24px", position: "relative" }}>
      <span style={{ position: "absolute", top: 12, right: 14, color: "#94A3B8", ...GM, fontSize: 9, fontWeight: 700, letterSpacing: "0.08em" }}>FUTURE CONCEPT — NOT AN AVAILABLE SERVICE</span>
      <h3 style={{ color: "#07111F", ...GF, fontSize: 16, fontWeight: 700, margin: "20px 0 10px" }}>{label}</h3>
      <div style={{ color: "#64748B", ...GF, fontSize: 14, lineHeight: 1.7 }}>{children}</div>
    </div>
  );
}
          ...
          <div key={stage.id} style={{ display: "flex", gap: 16, marginBottom: i < stages.length - 1 ? 24 : 0 }}>
            ...
            <div style={{ paddingBottom: i < stages.length - 1 ? 20 : 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 6 }}>
                <h3 style={{ color: "#07111F", ...GF, fontSize: 14, fontWeight: 700, margin: 0 }}>{stage.label}</h3>
...
export function EnotaryFaqGroup({ group }: { group: FaqGroup }) {
  ...
  return (
    <div style={{ marginBottom: 40 }}>
      <h2 style={{ color: "#07111F", ...GF, fontSize: "clamp(16px, 2.5vw, 21px)", fontWeight: 800, margin: "0 0 16px", paddingBottom: 12, borderBottom: "1px solid rgba(0,0,0,0.08)" }}>{group.title}</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        {group.items.map(({ id, q, a }) => {
          const expanded = open === id;
          return (
            <div key={id} style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 9, overflow: "hidden", boxShadow: "0 1px 4px rgba(7,17,31,0.06)" }}>
              <button
              ...
              >
                <span style={{ color: "#07111F", ...GF, fontSize: 14, fontWeight: 600 }}>{q}</span>
              ...
              <div id={`efaq-${id}`} hidden={!expanded} style={{ padding: "0 20px 16px" }}>
                <p style={{ color: "#64748B", ...GF, fontSize: 14, lineHeight: 1.7, margin: 0 }}>{a}</p>
              </div>
            </div>
          );
        })}
