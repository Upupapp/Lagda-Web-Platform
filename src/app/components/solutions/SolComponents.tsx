      ...
      <div style={{ display: "grid", gap: 6 }} className="doc-ex-grid">
        {docs.map((d) => (
          <div key={d} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
            ...
            <span style={{ color: "#64748B", fontFamily: "'Geist', sans-serif", fontSize: 13, lineHeight: 1.45 }}>{d}</span>
          </div>
        ))}
      </div>
...
export function WorkflowSteps({ steps, title }: { steps: WorkflowStep[]; title?: string }) {
  return (
    <div>
      {title && <p style={{ color: "#64748B", ...GM, fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 16 }}>{title}</p>}
      <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        {steps.map((s, i) => (
          <div key={i} style={{ display: "flex", gap: 14, position: "relative", paddingBottom: i < steps.length - 1 ? 20 : 0 }}>
            {i < steps.length - 1 && (
              <div style={{ position: "absolute", left: 18, top: 38, width: 1, height: "calc(100% - 18px)", background: "rgba(0,0,0,0.08)" }} />
            )}
            <div style={{
              width: 36, height: 36, borderRadius: "50%",
              background: "rgba(0,120,212,0.1)", border: "2px solid rgba(0,120,212,0.3)",
              display: "flex", alignItems: "center", justifyContent: "center",
              flexShrink: 0, zIndex: 1,
            }}>
              <span style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700 }}>{s.num}</span>
            </div>
            <div style={{ paddingTop: 7 }}>
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0, marginBottom: 2 }}>{s.label}</p>
...
export function NoticeBox({ label, text, color = "#C9960C" }: { label: string; text: string; color?: string }) {
  ...
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 12, padding: "16px 20px" }}>
      ...
      <p style={{ color: "#334155", ...GF, fontSize: 14, lineHeight: 1.7, margin: 0 }}>{text}</p>
    </div>
  );
}
...
export function SolLegalNote({ extra }: { extra?: string }) {
  return (
    <div style={{
      background: "#f8fafb", borderTop: "1px solid rgba(0,0,0,0.07)",
      padding: "20px 24px",
    }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <p style={{ color: "#94A3B8", ...GF, fontSize: 12, lineHeight: 1.6, margin: 0, marginBottom: extra ? 6 : 0 }}>
          {LEGAL_NOTE}
        </p>
        {extra && (
          <p style={{ color: "#64748B", ...GF, fontSize: 12, lineHeight: 1.6, margin: 0 }}>
            {extra}
          </p>
        )}
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {roles.map((r) => (
        <div key={r.role} style={{
          ...
          <span style={{ color: "#0078D4", ...GM, fontSize: 10, fontWeight: 700, minWidth: 140, flexShrink: 0 }}>{r.role.toUpperCase()}</span>
          <span style={{ color: "#64748B", ...GF, fontSize: 13, lineHeight: 1.45 }}>{r.desc}</span>
        </div>
      ))}
    </div>
  );
      ...
      {paths.map((p) => (
        <Link key={p.path} to={p.path} style={{ textDecoration: "none" }}>
          <div style={{
            ...
            <div>
              <p style={{ color: "#07111F", ...GF, fontSize: 13, fontWeight: 700, margin: 0 }}>{p.label}</p>
